...
class AuditEvent(Base):
    ...
    user_id = Column(PGUUID(as_uuid=True), nullable=True, index=True)  # Supabase user ID (no FK)
...
class RateLimit(Base):
    ...
    user_id = Column(PGUUID(as_uuid=True), nullable=True)  # Supabase user ID (no FK)
    ip_address = Column(String(45), nullable=True)
